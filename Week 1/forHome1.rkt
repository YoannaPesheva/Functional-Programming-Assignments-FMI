...
(define (rev n)
  (define (helper copy-n res)
    (cond
      [(zero? copy-n) res]
      [else
       (helper (quotient copy-n 10) (+ (remainder copy-n 10) (* res 10))) ; copy=n becomes n without the last digit, result needs to be multiplied by 10 in reverse + last digit added
      ]
      )
    )
